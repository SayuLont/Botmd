...
xfar.Instagram(args[1]).then(async data => {
                    ...
                    await sendFileFromUrl(from,data.thumbnail,txt,m).then(async res => {
...
break
             ...
             case 'playvideo': case 'ytmp4':
                            ...
                            if (Number(filesize) >= 100000) return sendFileFromUrl(from, thumb, `─ 「 PLAY VIDEO 」─\n\n*▢ Title :* ${title}\n*▢ ID :* ${yut.videos[0].videoId}\n*▢ URL :* ${a.data}\n\n─━─━「 ‼️ 」━─━─\n*DURASI MELEBIHI BATAS, ANDA BISA DOWNLOAD MELALUI URL DI ATAS*`, msg)
...
case 'tiktok2':
    ...
    await reply ('Process')
    data = await fetchJson(`https://api.lolhuman.xyz/api/tiktok?apikey=ItsukaAllan&url=${text}`)
result = `⚜️ *Nickname*: ${data.result.author.nickname}\n❤️ *Like*: ${data.result.statistic.diggCount}\n💬 *Komentar*: ${data.result.statistic.commentCount}\n🔁 *Share*: ${data.result.statistic.shareCount}\n🎞️ *Views*: ${data.result.statistic.playCount}\n?? *Desc*: ${data.result.title}`
tttt =  await getBuffer(data.result.thumbnail)
anu = await fetchJson(`http://hadi-api.herokuapp.com/api/tiktok?url=${text}`)
buff = await getBuffer (anu.result.video.nowm)


    await reply ('wait for a few minutes')
anu = `${result}`
message = await prepareWAMessageMedia({ video: buff, jpegThumbnail: tttt }, { upload: ky.waUploadToServer })
    template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
        templateMessage: {
            hydratedTemplate: {
                videoMessage: message.videoMessage,
...
m.reply('Process')
anu = `Done Masz`
   message = await prepareWAMessageMedia({ image: {url:'https://api.lolhuman.xyz/api/random/waifu?apikey=ItsukaAllan' }}, { upload: ky.waUploadToServer })
                 template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
                    templateMessage: {
                        hydratedTemplate: {
                            imageMessage: message.imageMessage,
...
case 'blok': {
		...
		await ky.updateBlockStatus(users, 'block').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
        case 'unblok': {
		if (!isCreator) throw mess.owner
		let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
		await ky.updateBlockStatus(users, 'unblock').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
...
case 'promote': {
	...
	break
 case 'sc':
    m.reply('github.com/rizkiadiasa/IkyOgiwara')
    break
case 'kick': {
		if (!m.isGroup) throw mess.group
                if (!isBotAdmins) throw mess.botAdmin
                if (!isGroupAdmins) throw mess.admin
		let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
		await ky.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
	break
          case 'add': {
		if (!m.isGroup) throw mess.group
		...
		await ky.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
	}
             break
             //Batas
            case 'chat': {
            ...
            case 'delete': case 'del': {
                ...
                ky.sendMessage(m.chat, { delete: id })
            }
            break
            case 'pinterest': {
                m.reply(mess.wait)
            ...
            case 'hentai': {
                ...
                anu = await hentai()
                result = anu[Math.floor(Math.random(), anu.length)]
                ky.sendMessage(m.chat, { video: { url: result.video_1 }, caption: `▹ Title : ${result.title}\n▹ Category : ${result.category}\n▹ Mimetype : ${result.type}\n▹ Views : ${result.views_count}\n▹ Shares : ${result.share_count}\n▹ Source : ${result.link}\n▹ Media Url : ${result.video_1}` }, { quoted: m })
            }
            break
            case 'quotesanime': case 'quoteanime': {
                ...
                result = anu[Math.floor(Math.random(), anu.length)]
                ky.sendMessage(m.chat, { text: `~_${result.quotes}\n\nBy '${result.karakter}', ${result.anime}\n\n- ${result.up_at}` })
            }
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'TEL;type=CELL;type=VOICE;waid=6285215319934:+62 852-1531-9933\n' // WhatsApp ID + phone number
                    + 'END:VCARD'
                ky.sendMessage(m.chat, { contacts: { displayName: 'Rizky Adiasa.', contacts: [{ vcard }] } }, { quoted: m })
            }
            break
            case 'eval': {
...
case 'tagall':
 ...
 arr.push(i.jid)
  ...
  break
            case 'bot': 
                teks = (`Hello @${sender.split("@")[0]} Bot Online`)
                ky.sendMessage(m.chat, {text:teks}, { quoted: m,contextInfo: {"mentionedJid": sender}})
            break
